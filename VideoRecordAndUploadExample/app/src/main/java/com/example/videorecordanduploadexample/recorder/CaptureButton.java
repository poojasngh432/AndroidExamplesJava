...
public class CaptureButton extends View {
    ...
    public CaptureButton(Context context, int size) {
        ...
        button_state = CameraView.BUTTON_STATE_BOTH;  //初始化按钮为可录制可拍照
        Log.d(TAG, "CaptureButton start");
        duration = 10 * 1000;              //默认最长录制时间为10s
        Log.d(TAG, "CaptureButton end");
    ...
    private void handlerUnpressByState() {
        ...
        switch (state) {
            //当前是点击按下
            case STATE_PRESS:
                if (captureLisenter != null && (button_state == CameraView.BUTTON_STATE_ONLY_CAPTURE || button_state == CameraView.BUTTON_STATE_BOTH)) {
